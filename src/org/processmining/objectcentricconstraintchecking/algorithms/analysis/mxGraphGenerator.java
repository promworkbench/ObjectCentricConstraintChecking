 ...
 */
package org.processmining.objectcentricconstraintchecking.algorithms.analysis;
...
import org.processmining.objectcentricconstraintchecking.algorithms.costfunction.mxDoubleValCostFunction;
import org.processmining.objectcentricconstraintchecking.algorithms.generatorfunction.mxGeneratorFunction;
import org.processmining.objectcentricconstraintchecking.algorithms.generatorfunction.mxGeneratorRandomFunction;
...
public class mxGraphGenerator
{
	...
	private static final double PORT_DIAMETER = 6;
	private static final double PORT_RADIUS = 3;
	 ...
	 */
	public void getTraceGraph(mxAnalysisGraph aGraph, Map procExeMap, ArrayList<ArrayList<Integer>> edgeLst)
	{
		if (procExeMap.size() == 0)
		{
			throw new IllegalArgumentException();
		}
		...
		mxGraph graph = aGraph.getGraph();
		Object parent = graph.getDefaultParent();
		Object[] vertices = new Object[procExeMap.size()];

		int i = 0;
		for (Object key : procExeMap.keySet()){
			vertices[i] = graph.insertVertex(parent, null, ((OcelEvent) procExeMap.get(key)).activity, 0, 0, 25, 25);
			i+=1;
		}

		System.out.println(edgeLst);

		// iterate the keymap to connect the vertices

		for (int j = 0; j < edgeLst.size(); j++)
		{
			graph.insertEdge(parent,
					null,
					getNewEdgeValue(aGraph),
					vertices[edgeLst.get(j).get(0)],
					vertices[edgeLst.get(j).get(1)]);

		}

//		for (String key : edgeMap.keySet()) {
//			System.out.println("key:"+key);
//			ArrayList<ArrayList<Integer>> edgeMapForThisTrace = edgeMap.get(key);
//
//			System.out.println(edgeMapForThisTrace);
//
//			for (ArrayList<Integer> al: edgeMapForThisTrace){
////				graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[al.get(0)], vertices[al.get(1)]);
//			}
//		}
	...
	public void getPathGraph(mxAnalysisGraph aGraph, int numVertices)
	{
		...
		for (int i = 0; i < numVertices - 1; i++)
		{
			graph.insertEdge(parent,
					null,
					getNewEdgeValue(aGraph), vertices[i], vertices[i + 1]);
		}
	};
	 ...
	 */
	public void getObjToObjGraph(mxAnalysisGraph aGraph)
	{

		mxGraph graph = aGraph.getGraph();
		Object parent = graph.getDefaultParent();
		Object[] vertices = new Object[12];

		// create nodes
		// object type X
		vertices[0] = graph.insertVertex(parent, "obj1", "Object type", 100, 100, 120, 80, "ellipse;fillColor=#FFA836");
		// object type Y
		vertices[1] = graph.insertVertex(parent, "obj2", "Object type", 100, 500, 120, 80, "ellipse;fillColor=#FFA836");
		// Activity A
		vertices[2] = graph.insertVertex(parent, "act1", "Activity", 500, 100, 120, 80, "rectangle;fillColor=#0ba8fb");
		// Activity B
		vertices[3] = graph.insertVertex(parent, "act2", "Activity", 500, 500, 120, 80, "rectangle;fillColor=#0ba8fb");
		// Temp constraint T
		vertices[4] = graph.insertVertex(parent, "tc", "Temp", 300, 300, 100, 100, "fillColor=#71C562");

		// add two ports to obj1
		mxGeometry geo0 = new mxGeometry(1,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo0.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo0.setRelative(true);
		mxCell port0 = new mxCell(null, geo0, "ellipse");
		port0.setVertex(true);

		mxGeometry geo1 = new mxGeometry(0.5, 1, PORT_DIAMETER,
				PORT_DIAMETER);
		geo1.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo1.setRelative(true);
		mxCell port1 = new mxCell(null, geo1, "ellipse");
		port1.setVertex(true);

		graph.addCell(port0, vertices[0]);
		graph.addCell(port1, vertices[0]);

		// add two ports to obj2
		mxGeometry geo2 = new mxGeometry(0.5,0, PORT_DIAMETER,
				PORT_DIAMETER);
		geo2.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo2.setRelative(true);
		mxCell port2 = new mxCell(null, geo2, "ellipse");
		port2.setVertex(true);

		mxGeometry geo3 = new mxGeometry(1,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo3.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo3.setRelative(true);
		mxCell port3 = new mxCell(null, geo3, "ellipse");
		port3.setVertex(true);

		graph.addCell(port2, vertices[1]);
		graph.addCell(port3, vertices[1]);

		// add two ports to act1
		mxGeometry geo4 = new mxGeometry(0,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo4.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo4.setRelative(true);
		mxCell port4 = new mxCell(null, geo4, "ellipse");
		port4.setVertex(true);

		graph.addCell(port4, vertices[2]);


		// add two ports to act2
		mxGeometry geo5 = new mxGeometry(0,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo5.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo5.setRelative(true);
		mxCell port5 = new mxCell(null, geo5, "ellipse");
		port5.setVertex(true);

		graph.addCell(port5, vertices[3]);

		// add two ports to temp2
		mxGeometry geo6 = new mxGeometry(0,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo6.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo6.setRelative(true);
		mxCell port6 = new mxCell(null, geo6, "ellipse");
		port6.setVertex(true);

		graph.addCell(port6, vertices[4]);

		// create edges
		mxCell edg1 = (mxCell) graph.insertEdge(parent, null, "ref", port0, port4,"strokeWidth=3;endArrow=none;dashed=1;strokeColor=#FFA836");
		mxCell edg2 = (mxCell) graph.insertEdge(parent, null, getNewEdgeValue(aGraph), port1, port2,"strokeWidth=5;endArrow=classic;strokeColor=#71C562");
		mxCell edg3 = (mxCell) graph.insertEdge(parent, null, "ref", port3, port5,"strokeWidth=3;endArrow=none;dashed=1;strokeColor=#FFA836");

		// let some elements be not connectable
		((mxCell)vertices[0]).setConnectable(false);
		((mxCell)vertices[1]).setConnectable(false);
		((mxCell)vertices[2]).setConnectable(false);
		((mxCell)vertices[3]).setConnectable(false);
		((mxCell)vertices[4]).setConnectable(false);

		// add one port to temp2
		mxGeometry geo7 = new mxGeometry(0,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo7.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo7.setRelative(true);
		mxCell port7 = new mxCell(null, geo7, "ellipse");
		port7.setVertex(true);
		graph.addCell(port7, edg2);

		mxCell edg4 = (mxCell) graph.insertEdge(parent, null, "ref", port7, port6,"line;strokeWidth=3;endArrow=none;dashed=1;strokeColor=#71C562");

	}

	public void getObjActCardinalityGraph(mxAnalysisGraph aGraph)
	{
		mxGraph graph = aGraph.getGraph();
		Object parent = graph.getDefaultParent();
		...
		// create nodes
		// object type X
		vertices[0] = graph.insertVertex(parent, "act", "Activity", 100, 100, 120, 80, "rectangle;fillColor=#0ba8fb");
		// object type Y
		vertices[1] = graph.insertVertex(parent, "obj", "Object type", 100, 300, 120, 80, "ellipse;fillColor=#FFA836");
		// Activity A

		// add two ports to act
		mxGeometry geo0 = new mxGeometry(0.5,1, PORT_DIAMETER,
				PORT_DIAMETER);
		geo0.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo0.setRelative(true);
		mxCell port0 = new mxCell(null, geo0, "ellipse");
		...
		graph.addCell(port1, vertices[1]);

		// create edges
		mxCell edg = (mxCell) graph.insertEdge(parent, null, getNewEdgeValue(aGraph), port0, port1,"strokeWidth=3;endArrow=none;strokeColor=#FFA836");
		...
		vertices[2] = graph.insertVertex(parent, "card-act", "1", 120, 190, 30, 30, "strokeColor=#FFFFFF;fillColor=#FFFFFF;fontSize=13");
		// cardinality value for
		vertices[3] = graph.insertVertex(parent, "card-obj", "1", 120, 270, 30, 30, "strokeColor=#FFFFFF;fillColor=#FFFFFF;fontSize=13");

	}

	public void getObjObjCardinalityGraph(mxAnalysisGraph aGraph)
	{
		mxGraph graph = aGraph.getGraph();
		Object parent = graph.getDefaultParent();
		Object[] vertices = new Object[12];
		...
		// create nodes
		// object type X
		vertices[0] = graph.insertVertex(parent, "obj1", "Object type", 100, 100, 120, 80, "rectangle;fillColor=#0ba8fb");
		// object type Y
		vertices[1] = graph.insertVertex(parent, "obj2", "Object type", 100, 500, 120, 80, "rectangle;fillColor=#0ba8fb");
		// Activity A
		vertices[2] = graph.insertVertex(parent, "act", "Activity", 300, 400, 120, 80, "rectangle;fillColor=#71C562");
		...
		// add 2 ports to act
		mxGeometry geo4 = new mxGeometry(0.5,0, PORT_DIAMETER,
				PORT_DIAMETER);
		geo4.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo4.setRelative(true);
		mxCell port4 = new mxCell(null, geo4, "ellipse");
		...
		graph.addCell(port4, vertices[3]);

		mxGeometry geo5 = new mxGeometry(0.5,1, PORT_DIAMETER,
				PORT_DIAMETER);
		geo5.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo5.setRelative(true);
		mxCell port5 = new mxCell(null, geo5, "ellipse");
		...
		// create edges
		mxCell edg1 = (mxCell) graph.insertEdge(parent, null, getNewEdgeValue(aGraph), port0, port4,"edgeStyle=none;strokeWidth=3;endArrow=none;dashed=1;strokeColor=#FFA836");
		mxCell edg2 = (mxCell) graph.insertEdge(parent, null, getNewEdgeValue(aGraph), port1, port2,"edgeStyle=none;strokeWidth=5;endArrow=classic;strokeColor=#71C562");
		mxCell edg3 = (mxCell) graph.insertEdge(parent, null, getNewEdgeValue(aGraph), port3, port5,"edgeStyle=none;strokeWidth=3;endArrow=none;dashed=1;strokeColor=#FFA836");
	...
	public void getObjToActGraph(mxAnalysisGraph aGraph)
	{
		mxGraph graph = aGraph.getGraph();
		Object parent = graph.getDefaultParent();
		Object[] vertices = new Object[12];
		...
		vertices[0] = graph.insertVertex(parent, "obj1", "Object type", 100, 100, 120, 80, "ellipse;fillColor=#FFA836");
		// object type Y
		vertices[1] = graph.insertVertex(parent, "act1", "Activity", 100, 500, 120, 80, "rectangle;fillColor=#0ba8fb");
		// Activity A
		vertices[2] = graph.insertVertex(parent, "tc", "Temp", 300, 300, 120, 80, "rectangle;fillColor=#71C562");
		// Activity B
		vertices[3] = graph.insertVertex(parent, "obj2", "Object type", 500, 300, 120, 80, "ellipse;fillColor=#FFA836");
		// Temp constraint T
		...
		mxCell port6 = new mxCell(null, geo6, "ellipse");
		port6.setVertex(true);
		graph.addCell(port6, vertices[2]);
		...
		mxCell edg4 = (mxCell) graph.insertEdge(parent, null, getNewEdgeValue(aGraph), port6, port7,"edgeStyle=none;endArrow=none;strokeWidth=3;dashed=1;strokeColor=#71C562");


	}

	public void getActToActGraph(mxAnalysisGraph aGraph)
	{
		mxGraph graph = aGraph.getGraph();
		Object parent = graph.getDefaultParent();
		Object[] vertices = new Object[12];
		...
		// create nodes
		// object type X
		vertices[0] = graph.insertVertex(parent, "act1", "Activity", 100, 100, 120, 80, "rectangle;fillColor=#0ba8fb");
		// object type Y
		vertices[1] = graph.insertVertex(parent, "act2", "Activity", 100, 500, 120, 80, "rectangle;fillColor=#0ba8fb");
		// Activity A
		vertices[2] = graph.insertVertex(parent, "tc", "Temp", 300, 300, 120, 80, "rectangle;fillColor=#71C562");
		// Activity B
		vertices[3] = graph.insertVertex(parent, "obj", "Object type", 500, 300, 120, 80, "ellipse;fillColor=#FFA836");
		// Temp constraint T
	...
	public void getActToActWithCoRefGraph(mxAnalysisGraph aGraph)
	{
		mxGraph graph = aGraph.getGraph();
		Object parent = graph.getDefaultParent();
		Object[] vertices = new Object[12];
		...
		// create nodes
		// object type X
		vertices[0] = graph.insertVertex(parent, "act1", "(unspecified)\n Activity", "act",100, 100, 120, 80, "rectangle;fillColor=#0ba8fb");		// object type Y
		vertices[1] = graph.insertVertex(parent, "act2", "(unspecified)\n Activity", "act",100, 500, 120, 80, "rectangle;fillColor=#0ba8fb");
		// Activity A
		vertices[2] = graph.insertVertex(parent, "obj1", "(unspecified)\n Object type", "obj", 500, 100, 120, 80, "ellipse;fillColor=#FFA836");
		// Activity B
		vertices[3] = graph.insertVertex(parent, "obj2", "(unspecified)\n Object type", "obj", 500, 500, 120, 80, "ellipse;fillColor=#FFA836");
		// Temp constraint T
		vertices[4] = graph.insertVertex(parent, "tc", "(unspecified)\n Temporal\n constraint", "tc", 300, 300, 100, 100, "rectangle;fillColor=#71C562");
		...
		// port on the right side
		mxGeometry geo2 = new mxGeometry(1,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo2.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo2.setRelative(true);
		mxCell port2 = new mxCell(null, geo2, "ellipse");
		...
		graph.addCell(port4, vertices[2]);
		graph.addCell(port5, vertices[2]);

		// add two ports to obj2

		// port on the top
		mxGeometry geo6 = new mxGeometry(0.5,0, PORT_DIAMETER,
				PORT_DIAMETER);
		geo6.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo6.setRelative(true);
		mxCell port6 = new mxCell(null, geo6, "ellipse");
		...
		graph.addCell(port6, vertices[3]);
		graph.addCell(port7, vertices[3]);

		// add two ports to obj2

		// port on the left
		mxGeometry geo8 = new mxGeometry(0,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo8.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo8.setRelative(true);
		mxCell port8 = new mxCell(null, geo8, "ellipse");
		port8.setVertex(true);
		graph.addCell(port8, vertices[4]);

		// create edges
		mxCell edg1 = (mxCell) graph.insertEdge(parent, null, "ref", port0, port4,"strokeWidth=3;endArrow=none;dashed=1;strokeColor=#FFA836");
		mxCell edg2 = (mxCell) graph.insertEdge(parent, null, getNewEdgeValue(aGraph), port1, port3,"strokeWidth=5;endArrow=classic;strokeColor=#71C562");
		mxCell edg3 = (mxCell) graph.insertEdge(parent, null, "ref", port2, port7,"strokeWidth=3;endArrow=none;dashed=1;strokeColor=#FFA836");
		...
		// add one port to temp2
		mxGeometry geo9 = new mxGeometry(0,0.5, PORT_DIAMETER,
				PORT_DIAMETER);
		geo9.setOffset(new mxPoint(-PORT_RADIUS, -PORT_RADIUS));
		geo9.setRelative(true);
		mxCell port9 = new mxCell(null, geo9, "ellipse");
		port9.setVertex(true);
		graph.addCell(port9, edg2);

		mxCell edg4 = (mxCell) graph.insertEdge(parent, null, "related", port5, port6,"strokeWidth=3;endArrow=none;strokeColor=#FFA836");
		mxCell edg5 = (mxCell) graph.insertEdge(parent, null, getNewEdgeValue(aGraph), port9, port8,"line;strokeWidth=3;endArrow=none;dashed=1;strokeColor=#71C562");
